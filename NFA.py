...
# M is a 5-tuple, (Q, Sigma, delta, q0, F), consisting of:
# a finite set of states Q
# a finite set of input symbols called the alphabet Sigma
# a transition function delta : Q x Sigma -> P(Q), where P(Q) is the power set of Q
# an initial (or start) state q0 in Q
# a set of accepting (or final) states F subset of Q


class NFAError(
    Exception):  # exception is a class that all built-in Python errors (like ValueError, TypeError) inherit from.
...
class InputStringError(Exception):
    # raised when there isn't an error with the NFA, but with the input string fed into it
    # (it contains characters not present in the NFA's alphabet)
    pass


# duplicate rule error seen in NFA's no longer found - the NFA transition function allow for multiple
# destination states for the same source state and symbol
...
def fixUtf8Corruption(possiblyCorruptedString):
    try:
        ...
        # if either encoding or decoding fails, the string is returned as it is.
...
def parseRules(ruleLines):
    ...
    for line in ruleLines:
        ...
        symbol = symbol.strip()
        symbol = fixUtf8Corruption(
            symbol)  # fixing any possible corruption that can be caused by file in different encoding from utf-8
...
def parseFile(inputNfaFile):
    sections = parser.parse_sections(inputNfaFile)
    states = sections.get("States", [])
    sigma = [fixUtf8Corruption(symbol) for symbol in sections.get("Sigma", [])]  # sigma = alphabet
    ...
    if len(startSection) == 0:
        ...
        raise UndefinedStartStateError("An NFA can only have one start state")

    accept = sections.get("Accept", [])  # a nfa can have multiple accept states
...
def isNfaValid(NFA):
    ...
    for sourceState in rules:  # rules dict key is the source state of the rule
        ...
        for symbol in rules[sourceState]:
            if symbol not in sigma and symbol not in ["epsilon",
                                                      "ε"]:  # epsilon doesn't need to be defined in the alphabet
                ...
                raise EpsilonTransitionError(
                    "Epsilon doesn't need to be defined in the alphabet for the NFA (it includes it by default). You can use 'epsilon' or 'ε' in your rules without defining epsilon or ε.")

            # for destinationState in rules[sourceState][symbol]: not for, as there is only one destinationState,
            # this for would split the destinationState string character by character
            for destinationState in list(
                    rules[sourceState][symbol]):  # all possible destination states from that source state, symbol tuple
                if destinationState not in states:
                    raise InvalidStateError(
                        f"Destination state {destinationState} {rules} is not defined in the states list for the NFA")
...
def getNextStates(currentState, currentSymbol, rules):
    if currentState not in rules:
        return set(
            [currentState])  # considered by default for every state, if a rule is not specified, the NFA stays in the same state
        # here we have no rule with the source state = the current state of the NFA
    elif currentSymbol not in rules[currentState]:
        return set(
            [currentState])  # same case, but in the NFA we have a rule defined with the current state as the source state
...
def getEpsilonStatesSetMaxDepth(epsilonDepth1StatesSet, rules):
    ...
    epsilonStates.update(
        epsilonDepth1StatesSet)  # initialise it with the epsilon states with depth = 1 from current states
    ...
    while addedEpsilonState:
        ...
        for state in epsilonStates:
            if state in rules and 'epsilon' in rules[state]:
                newEpsilonStates.update(
                    rules[state]['epsilon'])  # adding epsilon transitions first - by using sets we allow no duplicates
...
def runNfa(NFA, inputString, stringSeparator, printNFASteps=True):
    ...
    for currentSymbol in splitIncludingNoSeparator(inputString, stringSeparator):
        ...
        for state in currentStates:
            if state in rules and 'epsilon' in rules[state]:
                epsilonDepth1States.update(
                    rules[state]['epsilon'])  # adding epsilon transitions first - by using sets we allow no duplicates

        epsilonStates = getEpsilonStatesSetMaxDepth(epsilonDepth1States, rules)
        currentStates.update(
            epsilonStates)  # when reaching a possible epsilon transition, the NFA branch can either take it or not
        # in this set we will have all the branches - with epsilon transitions and without

        for state in currentStates:  # now we can iterate across all possible current states, including the epsilon paths
            destinationStates = getNextStates(state, currentSymbol,
                                              rules)  # all destination states for one of the current possible states
    ...
    epsilonStates = getEpsilonStatesSetMaxDepth(currentStates,
                                                rules)  # seperate check needed for states after last symbol from the input string is set
