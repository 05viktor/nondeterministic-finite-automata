...
if len(sys.argv) == 1:
    try:
        ...
        stringInput = input(
            "What separator is used between the symbols in the input file? (Space -> ' ', NoSeparator -> '', ; -> ';', etc.) ")
...
#
    ...
    raise NotEnoughArgummentsError(
        "You need to also give input file name as an argument, and optionally if you want to output all steps taken by the NFA (1/0 as third argument) ")
    ...
    try:
        ...
        if len(sys.argv) == 4:
            allowVerbosity = bool(
                int(sys.argv[3]))  # 1 or 0 if i want all intermediate steps printed to the output or not
        else:
